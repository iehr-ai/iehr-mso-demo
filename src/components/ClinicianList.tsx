...
import { useIEHR, useIEHRNavigate } from '@iehr/react';
import { Organization, Practitioner } from '@iehr/fhirtypes';
...
export function ClinicianList({ organization }: ClinicianListProps): JSX.Element {
  ...
  const loadClinicians = useCallback(async (): Promise<void> => {
    try {
      setIsLoading(true);
      const practitioners = await getEnrolledPractitioners(iehr, organization);
  ...
  const loadAvailableClinicians = useCallback(async (): Promise<void> => {
    try {
      ...
      const searchResult = await iehr.search('Practitioner', {
        _count: 100,
        _fields: 'name',
      });

      const allPractitioners = searchResult.entry?.map((e) => e.resource as Practitioner) ?? [];
      const enrolledPractitioners = await getEnrolledPractitioners(iehr, organization);

      // Filter out already enrolled practitioners and the current user
      const availablePractitioners = allPractitioners.filter(
        (practitioner) =>
          !enrolledPractitioners.some((p) => p.id === practitioner.id) && practitioner.id !== iehr.getProfile()?.id
      );
  ...
  const handleUnenroll = async (practitioner: Practitioner): Promise<void> => {
    try {
      await unEnrollPractitioner(iehr, practitioner, organization);
  ...
  const handleSelectionChange = async (selectedIds: string[]): Promise<void> => {
    ...
    try {
      ...
      for (const id of selectedIds) {
        const practitioner = await iehr.readResource('Practitioner', id);
    ...
    try {
      if (selectedClinicians.length > 0) {
        const results = await Promise.allSettled(
          selectedClinicians.map((practitioner) => enrollPractitioner(iehr, practitioner, organization))
        );
