...
import { Organization, Patient } from '@iehr/fhirtypes';
...
export function PatientList({ organization }: PatientListProps): JSX.Element {
  ...
  const loadPatients = useCallback(async (): Promise<void> => {
    try {
      setIsLoading(true);
      const patients = await getEnrolledPatients(iehr, organization);
  ...
  const loadAvailablePatients = useCallback(async (): Promise<void> => {
    try {
      ...
      const searchResult = await iehr.search('Patient', {
        _count: 100,
        _fields: 'name,meta',
      });

      const allPatients = searchResult.entry?.map((e) => e.resource as Patient) ?? [];
      const enrolledPatients = await getEnrolledPatients(iehr, organization);
  ...
  const handleUnenroll = async (patient: Patient): Promise<void> => {
    try {
      await unEnrollPatient(iehr, patient, organization);
  ...
  const handleSelectionChange = async (selectedIds: string[]): Promise<void> => {
    ...
    try {
      ...
      for (const id of selectedIds) {
        const patient = await iehr.readResource('Patient', id);
    ...
    try {
      ...
      if (selectedPatients.length > 0) {
        for (const patient of selectedPatients) {
          await enrollPatient(iehr, patient, organization);
          successCount++;
        }
