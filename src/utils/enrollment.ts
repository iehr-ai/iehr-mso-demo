...
import { createReference, IEHRClient, normalizeErrorString, getReferenceString, resolveId } from '@iehr/core';
...
export async function enrollPractitioner(
  ...
  // 1. Search for the practitioner's project membership, if there is no project membership, throw an error
  const projectMembershipSearch = await iehr.searchOne('ProjectMembership', {
  ...
  if (projectMembershipSearch) {
    ...
    if (!organizationExists) {
      const policy = await getAccessPolicyByName(iehr, 'Managed Service Organization Access Policy');
    ...
    // 4. Update the ProjectMembership resource
    try {
      const updatedResource = await iehr.updateResource(membershipResource);
 ...
 *     See docs: https://www.iehr.ai/docs/api/fhir/operations/patient-set-accounts
 *
 * @param iehr - The iEHR client.
...
export async function enrollPatient(
  ...
  try {
    // 4. Call the $set-accounts operation with the Parameters resource. It will update the patient resource and all other resources in the patient's compartment.
    await iehr.post(`fhir/R4/ai/iehr/Patient/${patient.id}/$set-accounts`, parameters);
  } catch (error) {
    throw new Error(normalizeErrorString(error));
  }
}
...
export async function unEnrollPractitioner(
  ...
  // 1. Search for the practitioner's project membership
  const membershipResource = await iehr.searchOne('ProjectMembership', {
    profile: getReferenceString(practitioner),
  });
  ...
  if (membershipResource) {
    ...
    // 3. Update the ProjectMembership resource
    try {
      await iehr.updateResource(membershipResource);
    } catch (error) {
      throw new Error(normalizeErrorString(error));
    }
  }
...
export async function getEnrolledPractitioners(
  ...
  // 1. Search for all ProjectMembership resources
  const memberships = await iehr.searchResources('ProjectMembership');
  ...
  if (practitionerRefs.length > 0) {
    // 3. Search for the Practitioner resources that have ProjectMembership resources with access to this organization
    const practitioners = await iehr.searchResources('Practitioner', {
 ...
 */
export async function getEnrolledPatients(iehr: IEHRClient, organization: Organization): Promise<Patient[]> {
  // 1. Invalidate the Patient search cache first
  iehr.invalidateSearches('Patient');

  // 2. Search for all patients with this organization in their compartment
  const patients = await iehr.searchResources('Patient', {
 ...
 */
async function getAccessPolicyByName(iehr: IEHRClient, name: string): Promise<AccessPolicy> {
  const policy = await iehr.searchOne('AccessPolicy', {
    name: name,
  });
