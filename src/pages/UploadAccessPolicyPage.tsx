...
import { AccessPolicy } from '@iehr/fhirtypes';
...
export function UploadAccessPolicyPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const checkPolicy = async (): Promise<void> => {
      try {
        const searchResult = await iehr.search('AccessPolicy', {
  ...
  const handleUpload = async (): Promise<void> => {
    ...
    try {
      if (existingPolicy) {
        ...
        const updated = await iehr.updateResource<AccessPolicy>({
          ...MSO_ACCESS_POLICY,
        ...
        // Create new policy
        const created = await iehr.createResource<AccessPolicy>(MSO_ACCESS_POLICY);
