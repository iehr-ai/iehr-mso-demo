...
import { useIEHR, Document } from '@iehr/react';
import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router';
import { Organization, AccessPolicy } from '@iehr/fhirtypes';
...
export function NewClinicianPage(): JSX.Element {
  ...
  useEffect(() => {
    const fetchOrgs = async (): Promise<void> => {
      const orgs = await iehr.search('Organization', {});
  ...
  const handleCreateClinician = async (): Promise<void> => {
    ...
    try {
      ...
      const policySearch = await iehr.search('AccessPolicy', {
      ...
      const result = await iehr.post('admin/projects/:projectId/invite', {
